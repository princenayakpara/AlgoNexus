...
import { login as backendLogin, googleLogin as googleBackendLogin } from '../services/api';
...
const LoginPage = () => {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      const { user, token } = await backendLogin(email, password);
      login(token, user);
      navigate('/dashboard');
    } catch (err) {
        ...
        <div className="flex-1 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
          <div className="sm:mx-auto sm:w-full sm:max-w-md">
            <h2 className="mt-6 text-center text-3xl font-extrabold text-white">
              Sign in to AlgoNexus
            </h2>
            <p className="mt-2 text-center text-sm text-slate-400">
              Or{' '}
              <Link to="/signup" className="font-medium text-neon-cyan hover:text-brand-400">
          ...
          <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div className="glass-card py-8 px-4 shadow-xl sm:rounded-2xl sm:px-10 border border-slate-700/50">
              ...
              <div className="mt-6">
                ...
                <div className="mt-6 flex justify-center w-full" id="google-login-container">
                  <GoogleLogin
